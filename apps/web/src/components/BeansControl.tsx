...
function beanRevisionKey(bean: BeansBean | null | undefined): string | null {
  if (!bean) {
    return null;
  }
  return `${bean.id}:${bean.etag}`;
}
...
export default function BeansControl({
  ...
  const [hydratedEditBeanRevision, setHydratedEditBeanRevision] = useState<string | null>(null);
  ...
  const selectedBeanRevision = useMemo(() => beanRevisionKey(selectedBean), [selectedBean]);
  ...
  useEffect(() => {
    ...
    if ((!selectedBeanId || !beanById.has(selectedBeanId)) && beans.length > 0) {
      setSelectedBeanId(beans[0]?.id ?? null);
    }
  }, [beanById, beans, open, paneMode, selectedBeanId]);

  useEffect(() => {
    if (paneMode !== "detail") return;
    if (selectedBeanRevision === hydratedEditBeanRevision) return;
    setEditForm(formFromBean(selectedBean));
    setHydratedEditBeanRevision(selectedBeanRevision);
  }, [hydratedEditBeanRevision, paneMode, selectedBean, selectedBeanRevision]);

  const invalidateBeans = () => invalidateBeansQueries(queryClient, { environmentId, cwd });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      startTransition(() => {
        ...
        setEditForm(formFromBean(result.bean));
        setHydratedEditBeanRevision(beanRevisionKey(result.bean));
        setPaneMode("detail");
